    ...
    <div className="min-h-screen bg-[#FFF8EE] flex items-center justify-center p-4" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
      <div className="w-full max-w-[400px]">
        ...
        <div className="flex flex-col items-center mb-6">
          <PawPrint className="w-8 h-8 text-[#D4A574] mb-2" />
          <h1 className="text-[22px] text-[#222]" style={{ fontWeight: 700 }}>회원가입</h1>
        </div>

        <div className="bg-white rounded-2xl border border-[#E8D5C0] p-6 space-y-4">
          <div>
            <label className="text-[13px] text-[#666] mb-1.5 block">닉네임</label>
            <div className="flex items-center gap-2 border border-[#DDD] rounded-lg px-3 py-2.5 bg-[#FFF8EE]">
          ...
          <div className="border border-[#EEE] rounded-lg p-3 space-y-2">
            <button className="flex items-center gap-2 w-full" onClick={toggleAll}>
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${agreements.every(Boolean) ? "bg-[#D4A574] border-[#D4A574]" : "border-[#CCC]"}`}>
            ...
            <div className="border-t border-[#EEE] pt-2 space-y-2">
              {terms.map((t, i) => (
                <button key={i} className="flex items-center gap-2 w-full" onClick={() => toggle(i)}>
                  <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${agreements[i] ? "bg-[#D4A574] border-[#D4A574]" : "border-[#CCC]"}`}>
          ...
          <button
            className="w-full py-3 bg-[#D4A574] text-white rounded-lg text-[15px] hover:bg-[#C4956A] transition-colors"
